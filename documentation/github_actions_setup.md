...
- `/opt/chronicleofthefall/.env`
- `/opt/chronicleofthefall/secrets/firebase-admin.prod.json`

And `/opt/chronicleofthefall/.env` should include (at minimum):
...
```bash
ssh-keygen -t ed25519 -C "github-actions-chronicleofthefall" -f ./chronicleofthefall-actions
```

This creates:

- `chronicleofthefall-actions`
  This is the private key. Keep it private.
- `chronicleofthefall-actions.pub`
...
Paste the full contents of `chronicleofthefall-actions.pub` as a new line, save, then run:
...
```bash
scp ./chronicleofthefall-actions.pub ubuntu@158.180.234.11:/tmp/chronicleofthefall-actions.pub
ssh ubuntu@158.180.234.11
mkdir -p ~/.ssh
chmod 700 ~/.ssh
cat /tmp/chronicleofthefall-actions.pub >> ~/.ssh/authorized_keys
chmod 600 ~/.ssh/authorized_keys
rm /tmp/chronicleofthefall-actions.pub
```
...
```bash
ssh -i ./chronicleofthefall-actions ubuntu@158.180.234.11
```
...
```bash
cat ./chronicleofthefall-actions
```
...
docker pull ghcr.io/<owner>/chronicleofthefall-backend:latest
docker pull ghcr.io/<owner>/chronicleofthefall-frontend:latest
```
...
6. Give it a name such as `chronicleofthefall-ghcr-read`
7. Set an expiration
8. Enable this scope:
   - `read:packages`
...
- `PREVIEW_VM_SSH_PRIVATE_KEY` = full contents of `chronicleofthefall-actions`
...
```text
ghcr.io/<lowercase-github-owner>/chronicleofthefall-backend:latest
ghcr.io/<lowercase-github-owner>/chronicleofthefall-frontend:latest
```
...
1. Open GitHub
2. Open the package page for `chronicleofthefall-backend`
...
5. Repeat for `chronicleofthefall-frontend`
...
docker pull ghcr.io/<lowercase-github-owner>/chronicleofthefall-backend:latest
docker pull ghcr.io/<lowercase-github-owner>/chronicleofthefall-frontend:latest
```
